...
- Problem framing for Reorg Handling: what state we need, what invariants we must keep.
...
Indexers are incomplete without reorg safety.

Problem statement:
Detect fork divergence and rollback/reapply canonical state.
...
Pointers/slices to canonical chain snapshots must be copied at rollback boundaries to prevent historical corruption.
