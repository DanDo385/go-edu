# 06: Manual ERC20 Calls with Selectors

## Core Concepts

- Problem framing for Manual ERC20 Calls with Selectors: what state we need, what invariants we must keep.
...
This introduces ABI mechanics before higher-level bindings.

Problem statement:
Call ERC20 read methods by hand-encoding selectors and decoding ABI payloads.
...
`&cfg.Contract` passes an address pointer value; Go is still pass-by-value, so only the pointer value is copied.
