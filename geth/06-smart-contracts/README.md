...
1. **Module 07 (eth-call)**: Doing the same interactions from Go using `go-ethereum/ethclient`
2. **Module 08 (abigen)**: Understanding `eth_call` vs SendTransaction at the RPC level
3. **Module 09 (events)**: Watching pending txs in the mempool and simulating execution

## Exercises
1. **Exercise 1**: Connect to dev chain and query an ERC20 token's totalSupply
2. **Exercise 2**: Create a contract object and call multiple view functions
3. **Exercise 3**: Send a transaction to transfer tokens (on dev chain)
4. **Exercise 4**: Decode Transfer events from a transaction receipt
5. **Exercise 5**: Identify common errors and their solutions

## Additional Resources
- [Geth Console Documentation](https://geth.ethereum.org/docs/interface/javascript-console)
- [ABI Specification](https://docs.soliditylang.org/en/v0.8.23/abi-spec.html)
