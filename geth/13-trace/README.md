...
- Problem framing for Execution Tracing: what state we need, what invariants we must keep.
...
Tracing gives causality tools before building explorer/indexer style systems.

Problem statement:
Run transaction traces and preserve raw JSON for downstream analysis.
...
`json.RawMessage` is `[]byte`; mutating a returned slice mutates shared backing memory unless explicitly copied.
