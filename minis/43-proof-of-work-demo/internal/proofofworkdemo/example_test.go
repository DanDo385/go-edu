...
func ExampleMiningProbability() {
	...
	// Time: 10 seconds
	// Probability of finding block: 91.30%
...
func Example_difficultyScaling() {
	...
	for difficulty := 1; difficulty <= 6; difficulty++ {
		...
		if expectedTime < 0.1 {
			...
			minutes := math.Floor((expectedTime/60)*10) / 10
			timeStr = fmt.Sprintf("%.1f min", minutes)
		} else {
			hours := math.Floor((expectedTime/3600)*10) / 10
...
func ExampleCalculatePoolRewardsSolution() {
	miners := []Miner{
		{ID: "Alice", Shares: 100},
		{ID: "Bob", Shares: 150},
...
func ExampleBuildMerkleRootSolution() {
	transactions := []string{
		"Alice -> Bob: 10 BTC",
		"Bob -> Charlie: 5 BTC",
		"Charlie -> Dave: 3 BTC",
...
func Example_networkHashRate() {
	...
	for _, diff := range difficulties {
		...
		if requiredHashRate < 1 {
			hashRateStr = "0 H/s"
		} else if requiredHashRate < 1000 {
			hashRateStr = fmt.Sprintf("%.1f H/s", requiredHashRate)
