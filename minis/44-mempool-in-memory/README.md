...
- The concrete problem in Mempool In Memory and the correctness invariants it depends on.
...
- Diagnose and implement Mempool In Memory patterns without relying on hidden framework behavior.
...
Problem statement:
This lesson turns the previous module's concepts into a reusable engineering pattern for mempool in memory.

At this point in the arc:
Lesson 44 introduces a sharper systems concern so later modules can assume this mental model is stable.
...
This module teaches you how to build an in-memory transaction mempool.

## Key Concepts

- **Transaction pool**: Pending transaction storage
- **Priority ordering**: Gas price sorting
- **Size limits**: Eviction policies
- **Nonce tracking**: Per-account transaction order
...
## Related Lessons
- Previous: `minis/43-proof-of-work-demo`
- Next: `minis/45-p2p-gossip-mock-network`
