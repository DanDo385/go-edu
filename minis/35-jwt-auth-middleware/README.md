...
- The concrete problem in Jwt Auth Middleware and the correctness invariants it depends on.
...
- Diagnose and implement Jwt Auth Middleware patterns without relying on hidden framework behavior.
...
Problem statement:
This lesson turns the previous module's concepts into a reusable engineering pattern for jwt auth middleware.

At this point in the arc:
Lesson 35 introduces a sharper systems concern so later modules can assume this mental model is stable.
...
This module teaches you how to build JWT authentication middleware.

## Key Concepts

- **JWT tokens**: JSON Web Tokens
- **Middleware**: HTTP handler wrapping
- **Claims validation**: Expiry, issuer checks
- **Token generation**: Signing JWTs

## How to Run

```bash
go run ./minis/35-jwt-auth-middleware/cmd/app/main.go
```

## Related Lessons
- Previous: `minis/34-rate-limiter-token-bucket`
- Next: `minis/36-caching-reverse-proxy`
