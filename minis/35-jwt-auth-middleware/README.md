...
```bash
go run ./cmd/app -list
go run ./cmd/app -fn "RequireRole" -in "Hello, 世界 👋"
go run ./cmd/dev
```
...
- `internal/jwtauthmiddleware/exercise.go`: implement the TODOs here
- `internal/jwtauthmiddleware/solution.reference.go`: full reference solution (tagged)
