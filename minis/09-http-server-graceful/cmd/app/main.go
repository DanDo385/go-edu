...
USAGE:
    ...
    go run ./cmd/app/main.go 9090
    go run ./cmd/app/main.go 8080 10s
    go run ./cmd/app/main.go 3000 30s

Arguments:
    [port]    - Server port, e.g., ":8080" or "8080" (default: ":8080")
    [timeout] - Graceful shutdown timeout, e.g., "5s", "30s" (default: "5s")
...
func main() {
	...
	if len(os.Args) > 2 {
		if t, err := time.ParseDuration(os.Args[2]); err == nil {
			shutdownTimeout = t
		}
	}
