...
/*
Reference Solution - WebSocket Chat Room with Hub and Rooms
==========================================================

gorilla/websocket: Upgrade HTTP to WebSocket. Hub: rooms map, broadcast; Room:
clients map, register/unregister/broadcast channels. Client: send chan, read
pump (->hub.broadcast), write pump (<-send). Ping/pong for keepalive. Mutex
for client map. CheckOrigin for CORS (dev: allow all).
*/
...
func (c *Client) ReadPump() {
	...
	for {
		...
		fullMessage, _ := json.Marshal(map[string]string{
			"content": fmt.Sprintf("%s: %s", c.username, incoming.Content),
		})
