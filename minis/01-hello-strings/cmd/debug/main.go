...
func main() {
	...
	// Test TitleCase
	// Set breakpoint in exercise.go at TitleCase function
	// Expected: "hello world" → "Hello World"
	fmt.Println("--- Testing TitleCase ---")
	result1 := exercise.TitleCase(input)
	fmt.Printf("TitleCase(%q) = %q\n\n", input, result1)

	// Test Reverse
	// Set breakpoint in exercise.go at Reverse function
	// Expected: "hello world" → "dlrow olleh"
	fmt.Println("--- Testing Reverse ---")
	result2 := exercise.Reverse(input)
	fmt.Printf("Reverse(%q) = %q\n\n", input, result2)

	// Test RuneLen
	// Set breakpoint in exercise.go at RuneLen function
	// Expected: "hello world" → 11 runes
	fmt.Println("--- Testing RuneLen ---")
	result3 := exercise.RuneLen(input)
