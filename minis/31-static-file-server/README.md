...
- The concrete problem in Static File Server and the correctness invariants it depends on.
...
- Diagnose and implement Static File Server patterns without relying on hidden framework behavior.
...
Problem statement:
This lesson turns the previous module's concepts into a reusable engineering pattern for static file server.

At this point in the arc:
Lesson 31 introduces a sharper systems concern so later modules can assume this mental model is stable.
...
```bash
go run ./minis/31-static-file-server/cmd/app/main.go
```

## Related Lessons
- Previous: `minis/30-build-tags-conditional-compilation`
- Next: `minis/32-websocket-chatroom`
