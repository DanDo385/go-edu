...
USAGE:
    go run ./cmd/grpc-telemetry-service/main.go
    go run ./cmd/grpc-telemetry-service/main.go -window=10m
    go run ./cmd/grpc-telemetry-service/main.go -points=10 -window=5m
    go run ./cmd/grpc-telemetry-service/main.go -points=100 -metrics=5
...
func main() {
	...
	// DEBUG: In Variables panel, you'll see: window=5m, numPoints=5, numMetrics=2

	window := flag.Duration("window", 5*time.Minute, "Time window for aggregation")
	numPoints := flag.Int("points", 5, "Number of data points to generate")
	numMetrics := flag.Int("metrics", 2, "Number of different metrics to generate")
	seed := flag.Int64("seed", time.Now().UnixNano(), "Random seed for data generation")
	...
	// DEBUG: Hover over *numPoints to see the dereferenced int value
	...
	for i := 0; i < *numPoints; i++ {
		...
		if *verbose {
			log.Printf("Generated: metric=%s, value=%.2f, timestamp=%d",
	...
	for i, p := range points {
		...
		if *verbose {
			log.Printf("Pushed point %d: %s=%.2f", i+1, p.Metric, p.Value)
		}
	}
