...
USAGE:
    ...
    go run ./cmd/app/main.go "https://httpbin.org/delay/1"
    go run ./cmd/app/main.go "https://httpbin.org/get" 5 10s
    go run ./cmd/app/main.go "https://httpbin.org/status/500" 3 5s 200ms

Arguments:
    [url]        - URL to fetch (default: "https://httpbin.org/get")
    [retries]    - Maximum number of retry attempts (default: 3)
    [timeout]    - HTTP request timeout, e.g., "5s", "10m" (default: "5s")
    [delay]      - Base delay for exponential backoff, e.g., "100ms" (default: "100ms")
...
func main() {
	...
	if len(os.Args) > 2 {
		if r, err := strconv.Atoi(os.Args[2]); err == nil {
			maxRetries = r
		}
	}
	if len(os.Args) > 3 {
		if t, err := time.ParseDuration(os.Args[3]); err == nil {
			timeout = t
		}
	}
	if len(os.Args) > 4 {
