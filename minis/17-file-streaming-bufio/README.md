...
```bash
go run ./cmd/app -list
printf "hello\nworld\n" | go run ./cmd/app -fn "CountLines" -stdin
printf "hello\nworld\n" | go run ./cmd/app -fn "WordFrequency" -stdin
go run ./cmd/dev
```
...
- `internal/filestreamingbufio/exercise.go`: implement the TODOs here
- `internal/filestreamingbufio/solution.reference.go`: full reference solution (tagged)
