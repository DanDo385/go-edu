...
import (
	...
	"github.com/example/go-10x-minis/minis/19-channels-basics/internal/exercise"
)
...
func main() {
	...
	// Set breakpoint in exercise.go at Ping function
	ch := exercise.Ping(value)

	result := <-ch
	fmt.Printf("Received: %d\n", result)

	// Test PingPong
	in, out := exercise.PingPong(3)
	in <- 1
	in <- 2
	in <- 3
	close(in)
