...
- The concrete problem in Rate Limiter Token Bucket and the correctness invariants it depends on.
...
- Diagnose and implement Rate Limiter Token Bucket patterns without relying on hidden framework behavior.
...
Problem statement:
This lesson turns the previous module's concepts into a reusable engineering pattern for rate limiter token bucket.

At this point in the arc:
Lesson 34 introduces a sharper systems concern so later modules can assume this mental model is stable.
...
This module teaches you how to implement a token bucket rate limiter.

## Key Concepts

- **Token bucket algorithm**: Tokens replenish over time
- **Rate limiting**: Controlling request rate
- **Burst handling**: Allowing temporary spikes
- **golang.org/x/time/rate**: Standard library rate limiter
...
## Related Lessons
- Previous: `minis/33-tcp-echo-server-client`
- Next: `minis/35-jwt-auth-middleware`
