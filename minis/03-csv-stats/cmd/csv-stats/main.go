...
USAGE:
    go run ./cmd/csv-stats/main.go
    go run ./cmd/csv-stats/main.go -file=testdata/transactions.csv
    go run ./cmd/csv-stats/main.go -file=custom.csv -sort=sum
    go run ./cmd/csv-stats/main.go -sort=count -reverse
...
PACKAGE STRUCTURE:
- This file (cmd/csv-stats/main.go) is package main
...
func main() {
	...
	// DEBUG: In Variables panel, you'll see: file="../../testdata/transactions.csv"

	// File path for CSV input
	filePath := flag.String("file", "../../testdata/transactions.csv", "Path to CSV transactions file")

	// Sort order: none, count, sum, avg, category
	sortBy := flag.String("sort", "category", "Sort results by: none, count, sum, avg, category")

	// Reverse sort order
	reverse := flag.Bool("reverse", false, "Reverse sort order")

	// Show detailed output
	verbose := flag.Bool("verbose", false, "Show detailed per-category statistics")
	...
	// DEBUG: Watch 'filePath', 'sortBy', 'reverse', 'verbose', 'useSolution'
